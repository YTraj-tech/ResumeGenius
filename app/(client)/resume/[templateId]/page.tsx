...
//             <input id="sidebar-toggle" type="button" className="drawer-toggle" />
//             <div className="drawer-side lg:drawer-open z-50">
//               <label htmlFor="sidebar-toggle" className="drawer-overlay lg:hidden"></label>
//               <div className="bg-white h-full p-4 w-80 min-h-screen lg:min-h-0 lg:relative lg:shadow-md">
//                 <ResumeBuilderSidebar resumeId={resume.id} userId={userId} />
//               </div>
//             </div>
//           </div>
...
import { prisma } from "@/lib/prisma";
import { getUserResume } from "@/lib/resume-service";
import ResumeLayout from "@/components/Aditional_info/ResumeLayout";
...
export default async function ResumePage({ params }: ResumePageProps) {
  ...
  if (!user) redirect("/extract");

  const { templateId } = await params;
  const resumeData = await getUserResume(userId);

  if (!resumeData) redirect("/extract");
